...
export default function Checkout() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      clearCart()
      router.push(`/order-success?orderId=${placedOrder.id || placedOrder._id}`)
    } catch (err) {
          ...
          <div className="space-y-6">
            <div className="rounded-[2rem] border border-white/30 bg-white/90 p-8 shadow-xl backdrop-blur-xl">
              ...
              <p className="text-sm text-charcoal/70">
                Securely confirm your order and personalize the delivery details.
              </p>
            </div>

            <form
              onSubmit={handleSubmit}
              className="rounded-[2rem] border border-white/30 bg-cream/90 p-8 shadow-xl backdrop-blur-xl"
            >
              <div className="grid gap-6 md:grid-cols-2">
                <label className="space-y-2 text-sm text-charcoal">
                  ...
                  <input
                    ...
                    className="w-full rounded-3xl border border-brown/20 bg-white px-4 py-3 text-sm outline-none focus:border-brown"
                  />
                </label>
                <label className="space-y-2 text-sm text-charcoal">
                  Email address
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-8 w-full rounded-full bg-brown px-6 py-4 text-base font-semibold text-white shadow-xl hover:bg-dark-brown transition-colors disabled:opacity-60"
              >
